...
class OffenceListScreen extends StatefulWidget {
  ...
  @override
  State<OffenceListScreen> createState() => _OffenceListScreenState();
}

class _OffenceListScreenState extends State<OffenceListScreen> {
  DateTime _startDate = DateTime.now().subtract(const Duration(days: 1));
  DateTime _endDate = DateTime.now();
  final TextEditingController _searchController = TextEditingController();
  String _searchQuery = '';

  final List<Map<String, String>> _allOffences = [
    {'plate': 'QAX3967', 'time': '10:51 AM', 'desc': '10 (1) Without displaying coupon(s)', 'loc': 'Jalan Pedada'},
    {'plate': 'S55880Q', 'time': '10:42 AM', 'desc': '10 (1) Without displaying coupon(s)', 'loc': 'Jalan Pedada'},
    {'plate': 'BBL1234', 'time': '09:15 AM', 'desc': '08 - Parking in loading zone', 'loc': 'Jalan Maju'},
    {'plate': 'XYZ9876', 'time': '08:30 AM', 'desc': '04 - Parking outside marked space', 'loc': 'Jalan Awang'},
  ];

  List<Map<String, String>> get _filteredOffences {
    if (_searchQuery.isEmpty) return _allOffences;
    return _allOffences.where((item) {
  ...
  Widget build(BuildContext context) {
    final filteredList = _filteredOffences;
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return _buildOffenceCard(
                  context: context,
                  plate: item['plate']!,
                  location: item['loc']!,
                  offenceDescription: item['desc']!,
                  time: item['time']!,
                  index: index,
                );
              },
  ...
  Widget _buildOffenceCard({
    ...
    required String offenceDescription,
          ...
          MaterialPageRoute(
            builder: (context) => OffenceDetailScreen(
              data: {
                ...
                'dateTime': '${_formatDate(DateTime.now())}, $time',
                'imageUrl': 'https://picsum.photos/seed/${plate.replaceAll(' ', '')}/400/300',
                'desc': offenceDescription,
              },
            ),
          ),
        );
                ...
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => FullScreenImageViewer(
                      imageUrl: 'https://picsum.photos/seed/${plate.replaceAll(' ', '')}/400/300',
                      heroTag: 'offence_thumbnail_$plate',
                    ),
                  ),
                );
              },
              child: Hero(
                tag: 'offence_thumbnail_$plate',
                child: ClipRRect(
